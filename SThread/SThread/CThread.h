...
public:
	...
	BOOL  Alert();
protected:
	virtual void Run(LPVOID lpParamter=0)=0;
	LPVOID  lpParameter;

private:
	static DWORD WINAPI StartAddress(LPVOID lpParamater);
	HANDLE hThread;
	DWORD  dwThreadId;
	DWORD  dwState;
	void   *lpUserData;
...
//Identifier of thread which would be used to operation upon threads,eg,priority
